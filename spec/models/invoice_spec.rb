...
RSpec.describe Invoice, type: :model do
  ...
  describe 'class methods' do
    describe 'empty_invoices' do
      ...
      it 'returns all invoice records that do not have any items on it' do
        expect(Invoice.empty_invoices([invoice_1.id, invoice_2.id])).to eq([invoice_2.id])
        expect(Invoice.empty_invoices([invoice_1.id, invoice_3.id])).to eq([])
        expect(Invoice.empty_invoices([invoice_4.id, invoice_5.id])).to eq([invoice_5.id])
      end
    end
  end
end